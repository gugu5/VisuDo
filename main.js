...
d3.dsv(';', './FichierCompletBaseANONYME.csv', d3.autoType).then(data => {
    ...
    graphGeneric('graph2', columnNiveauActuel, (currentSelection, categoryField) => {
        ...
        // ça a demandé de tout questionner la structure de données (map, internmap, array, le rollup, etc...)
            ...
            'Ouverture' : "On voit une scission entre les groupes (1-2) et les groupes (3-4). Les régionaux/nationaux ont une ouverture relativement faible comparée à celle des rameurs de l'équipe suisse et des super-élites. La littérature va aussi dans ce sens ou considère l'ouverture comme non-significative dans la prédiction de la performance (selon les études). En aucun cas l'ouverture a été observée comme une dimension délétère à la performance.",
            ...
            'Extraversion' : "Ce graph fait ressortir de grandes différences d'extraversion entre les groupes. En réalité, l'échelle de l'axe Y s'adapte aux valeurs qui sont ici assez denses entre les groupes. Contrairement à ce qui saute aux yeux ici, il n'y a pas de différences majeures d'extraversion à observer.",
            'Agréabilité' : "Ce graph fait ressortir de grandes différences d'agréabilité entre les groupes. En réalité, l'échelle de l'axe Y s'adapte aux valeurs qui sont ici assez denses entre les groupes. Contrairement à ce qui saute aux yeux ici, il n'y a pas de différences majeures d'agréabilité à observer",
            'Stabilité Emotionnelle' : "On voit une scission entre les groupes (1-2) et les groupes (3-4). Les régionaux/nationaux ont une stabilité émotionnelle relativement faible comparée à celle des rameurs de l'équipe suisse et des super-élites. La littérature va aussi dans ce sens, la stabilité émotionnelle est une dimension majeure dans la performance (spécialement sa composante 'confiance en soi'). On pourrait se demander pourquoi le groupe 3 a une valeur moyenne supérieure au groupe 4; il faut garder à l'esprit que l'échantillon des super-élites est moins nombreux. De par son petit nombre, le groupe 4 est donc plus sujet à des variations intra-groupe.",
        }, 
